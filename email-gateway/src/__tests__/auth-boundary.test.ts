...
function config(adminToken = 'admin-secret'): GatewayConfig {
  return {
    ...
    amp: {
      apiKey: 'amp-key',
      agentAddress: 'email-bot@example.aimaestro.local',
      maestroUrl: 'http://127.0.0.1:1',
      defaultAgent: 'ops@example.aimaestro.local',
      tenant: 'example',
      inboxDir: '/tmp/email-inbox',
    },
    mandrill: {
      apiKey: 'mandrill-key',
      webhookKeys: { example: 'webhook-key' },
      allowedFromDomains: [],
    ...
    storage: {
      attachmentsPath: '/tmp/email-attachments',
    },
    adminToken,
    emailBaseDomain: 'example.com',
  };
}

const securityConfig: SecurityConfig = { operatorEmails: [] };

describe('Email gateway auth boundary', () => {
  ...
  it('keeps /health and /inbound public and rejects unauthenticated or malformed /api requests', async () => {
    const { server, baseUrl } = await listen(createHttpApp(config(), securityConfig));
    try {
      const health = await fetch(`${baseUrl}/health`);
      assert.equal(health.status, 200);

      const inbound = await fetch(`${baseUrl}/inbound`, { method: 'HEAD' });
